...
public class Solver {
	...
	public String solve() {
		if(!hm.isGreen()) { // kartan f쉜 stor f쉜 hashMapper
			return "";
		}
		Map finalMap = BFS();
	...
	private String backtrack(Map finalMap) {
		...
		try {
			...
			//Lugna er, hittat f철rsta kartan bara..
		}
		try {
	...
	public Map BFS() {
		...
		while (!prioQueue.isEmpty()) {
			...
			//
			if(!curr.evaluated){
				curr.evaluateMap(); //s채tt ett v채rde p책 br채det om det inte redan finns!
			}
			moves = curr.getMoves();
			for (Move m : moves) {
				Map nextMap = new Map(curr, m); //Skapa en ny karta, v채rdet av den ber채knas via konstruktorn
				nextMap.evaluateMap();
				if(nextMap.isWon()){
					return nextMap;
				}
