...
First we check whether the integer received is prime or not. If it is not we use Trial Division, sec. \ref{trial_division}, with the first 1000 primes to attempt to quickly break down the given integer $N$ into smaller components. This is to reduce the magnitude of $N$ and thus lessen the time spent factoring. If it is prime it is stored as the only factor of itself and the program stops.\\\\
